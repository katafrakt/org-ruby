...
module OrgRuby
  ...
  class OutputBuffer
    ...
    def initialize(output)
      ...
      @logger.level = if ENV["DEBUG"] || $DEBUG
        Logger::DEBUG
      else
        Logger::WARN
      end
    ...
    def insert(line)
      ...
      @logger.debug "Looking at #{line.paragraph_type}|#{line.assigned_paragraph_type}(#{current_mode}) : #{line}"
      ...
      if line.assigned_paragraph_type == :comment
        # Don't add to buffer
      elsif line.title?
      ...
      if mode_is_code?(current_mode) && !line.block_type
        ...
        if line.paragraph_type != :blank
          @code_block_indent = if @code_block_indent
            [@code_block_indent, line.indent].min
          else
            line.indent
          end
        end
    ...
    def get_next_headline_number(level)
      ...
      while level > @headline_number_stack.length
    ...
    def boundary_of_block?(line)
      ...
      true if mode_is_block? @output_type
    end
    ...
    def maintain_mode_stack(line)
      ...
      if (line.paragraph_type != :blank) ||
        ...
        while !@list_indent_stack.empty? &&
          ...
          if (@list_indent_stack.last == line.indent) &&
